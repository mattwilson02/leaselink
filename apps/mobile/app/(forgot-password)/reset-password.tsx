...
const SetPassword = () => {
	...
	useEffect(() => {
		if (params.error === 'INVALID_TOKEN' || !token) {
			Alert.alert(errorT('error'), errorT('reset_link_invalid'))
			router.replace('/(forgot-password)/enter-email')
		}
	}, [params, router, token, errorT])
	...
	const { handleSubmit, Field } = useForm({
		...
		onSubmit: async (values) => {
			...
			if (error) {
				Alert.alert(errorT('error'), error.message)
				return
			}
	...
	return (
		<Layout.SafeAreaView testID='reset-password-screen'>
				...
				>
					...
					>
						...
						<View style={styles.formContainer}>
							...
							>
								<Button.Text>{t('save_new_password')}</Button.Text>
							</Button.Root>
						</View>
					</Layout.KeyboardDismiss>
				</Layout.ScrollView>
