...
type Variant = 'primary' | 'secondary' | 'tertiary' | 'outline' | 'ghost' | 'link' | 'destructive'
...
const getContainerStyle = (variant: Variant, color: Color): ViewStyle => {
  ...
  if (variant === 'secondary') {
    if (color === 'error') return { backgroundColor: colors.error[100], borderWidth: 0 }
    if (color === 'sf-green') return { backgroundColor: colors.neutral['20'], borderWidth: 0 }
  ...
  // primary / default
  if (color === 'neutral') return { backgroundColor: colors.neutral['700'], borderWidth: 0 }
  if (color === 'sf-green') return { backgroundColor: colors.neutral['800'], borderWidth: 0 }
  if (color === 'error') return { backgroundColor: colors.error[600], borderWidth: 0 }
...
const sizeContainerStyles: Record<Size, ViewStyle> = {
  xs: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: borderRadius.sm },
  sm: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: borderRadius.md },
  md: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: borderRadius.md },
  lg: { paddingHorizontal: 16, paddingVertical: 12, borderRadius: borderRadius.md },
}
...
const ButtonText = ({ style, children }: ButtonTextProps) => {
  ...
  return (
    <Text
      style={[
        styles.text,
        { color: _currentTextColor, fontSize: sz.fontSize, lineHeight: sz.lineHeight },
        style,
      ]}
    >
      {children}
