...
describe('MarkOverduePaymentsController (E2E)', () => {
	...
	it('[POST] /payments/mark-overdue - should mark past-due payments as overdue', async () => {
		const { jwt } = await jwtFactory.makeJwt(false)

		// Create a PENDING payment with a past due date
		...
		expect(response.body.overdueCount).toBeDefined()
		expect(typeof response.body.overdueCount).toBe('number')
		expect(response.body.overdueCount).toBeGreaterThanOrEqual(1)
	})

	it('[POST] /payments/mark-overdue - should reject CLIENT auth', async () => {
