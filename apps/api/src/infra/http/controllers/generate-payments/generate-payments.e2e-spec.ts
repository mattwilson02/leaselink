...
describe('GeneratePaymentsController (E2E)', () => {
	...
	it('[POST] /payments/generate - should generate payments for active lease', async () => {
		...
		expect(response.body.paymentsGenerated).toBeDefined()
		expect(typeof response.body.paymentsGenerated).toBe('number')
		expect(response.body.paymentsGenerated).toBeGreaterThan(0)
	})

	it('[POST] /payments/generate - should return 400 for non-active lease', async () => {
		const { jwt } = await jwtFactory.makeJwt(false)
		...
		await request(app.getHttpServer())
			...
			.send({ leaseId: pendingLease.id })
			.expect(400)

		await prisma.lease.delete({ where: { id: pendingLease.id } })
	})

	it('[POST] /payments/generate - should reject CLIENT auth', async () => {
