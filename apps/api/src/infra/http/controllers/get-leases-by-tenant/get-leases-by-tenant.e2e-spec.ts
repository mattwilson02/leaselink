...
describe('GetLeasesByTenantController (E2E)', () => {
	...
	it('[GET] /leases/tenant - should return own leases for CLIENT', async () => {
		...
		const leaseIds = response.body.data.map((l: any) => l.id)
		expect(leaseIds).toContain(lease.id)
	})

	it('[GET] /leases/tenant - should reject EMPLOYEE auth', async () => {
